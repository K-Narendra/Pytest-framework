...
class ObjectActions:
    ...
    def click_object(driver, locator):
        WebDriverWait(driver, profile.MAX_TIMEOUT).until(ec.visibility_of_element_located(locator)).click()

    @staticmethod
    def click_using_javascript(driver, locator):
        element = WebDriverWait(driver, profile.MAX_TIMEOUT).until(ec.presence_of_element_located(locator))
        driver.execute_script("arguments[0].click();", element)

    @staticmethod
    def set_text(driver, locator, text):
        ObjectActions.clear_text(driver, locator)
        WebDriverWait(driver, profile.MAX_TIMEOUT).until(ec.visibility_of_element_located(locator)).send_keys(text)

    @staticmethod
    def clear_text(driver, locator):
        element = WebDriverWait(driver, profile.MAX_TIMEOUT).until(ec.visibility_of_element_located(locator))
    ...
    def select_first_value_from_dropdown(driver, locator):
        select = Select(
            WebDriverWait(driver, profile.MAX_TIMEOUT).until(ec.visibility_of_element_located(locator)))
    ...
    def wait_for_element_to_be_clickable(driver, locator):
        element = WebDriverWait(driver, profile.MAX_TIMEOUT).until(ec.element_to_be_clickable(locator))
        return bool(element)
    ...
    def switch_to_frame(driver, locator):
        WebDriverWait(driver, profile.MAX_TIMEOUT).until(
